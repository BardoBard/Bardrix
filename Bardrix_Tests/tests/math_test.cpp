...
TEST(math, degrees_to_radians) {
    ...
    EXPECT_EQ(bardrix::degrees_to_radians(90), M_PI / 2);
    EXPECT_EQ(bardrix::degrees_to_radians(180), M_PI);
    EXPECT_EQ(bardrix::degrees_to_radians(270), 3 * M_PI / 2);
    EXPECT_EQ(bardrix::degrees_to_radians(360), 2 * M_PI);
}
...
TEST(math, degrees_to_radians_degenerate) {
    ...
    EXPECT_EQ(bardrix::degrees_to_radians(-180), -M_PI);
    EXPECT_EQ(bardrix::degrees_to_radians(-270), -3 * M_PI / 2);
    EXPECT_TRUE(bardrix::nearly_equal(bardrix::degrees_to_radians(4345923), 4345923 * M_PI / 180));
    EXPECT_TRUE(bardrix::nearly_equal(bardrix::degrees_to_radians(-239858), -239858 * M_PI / 180));
}

/// \brief Test the radians to degrees
TEST(math, radians_to_degrees) {
    EXPECT_EQ(bardrix::radians_to_degrees(0), 0);
    EXPECT_EQ(bardrix::radians_to_degrees(M_PI / 2), 90);
    EXPECT_EQ(bardrix::radians_to_degrees(M_PI), 180);
    EXPECT_EQ(bardrix::radians_to_degrees(3 * M_PI / 2), 270);
    EXPECT_EQ(bardrix::radians_to_degrees(2 * M_PI), 360);
}
...
TEST(math, radians_to_degrees_degenerate) {
    ...
    EXPECT_EQ(bardrix::radians_to_degrees(-M_PI), -180);
    EXPECT_EQ(bardrix::radians_to_degrees(-3 * M_PI / 2), -270);
    EXPECT_EQ(bardrix::radians_to_degrees(4345923), 4345923 * 180 / M_PI);
    EXPECT_EQ(bardrix::radians_to_degrees(-239858), -239858 * 180 / M_PI);
}
